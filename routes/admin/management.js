...
router.post('/opozori-delodajalca',function(request,response) {
    ...
    console.log(err);
    if (err) response.send({uspelo:false});  //opozorjen na 1
...
router.post('/prekini-prijavo-dela',function(request,response) {
    ...
    if (err) response.send({uspelo:false});
        ...
        if (err) response.send({uspelo:false});  //opozorjen na 1
            ...
            console.log(err);
            if (err) response.send({uspelo:false});  //opozorjen na 0
            else {
...
router.post('/dokoncno-blokiraj-delavca',function(request,response) {
    ...
    if (err) response.send({uspelo:false});
        ...
        if (err) response.send({uspelo:false})
                    ...
                    connection.query("INSERT INTO delavci_eposta_blacklist (eposta) (SELECT eposta FROM delavci WHERE iddelavca = ?)",[request.body.iddelavca],function(err,results,fields) {
                        console.log(err);
                        if (err) response.send({uspelo:false});
                        else {
...
if (err) response.send({uspelo:false});
    ...
    if (err) response.send({uspelo:false})
        ...
        connection.query("UPDATE delavci SET opozorjen = NULL,odgovor_delavca=NULL,admin_pojasnilo=NULL WHERE iddelavca = ?",[request.body.iddelavca],function(err,results,fields) {
            if (err) response.send({uspelo:false});
...
router.post('/blokiraj-podjetje',function(request,response) {
    ...
    router.post('/dokoncno-blokiraj-podjetje',function(request,response) {
        ...
        if (err) response.send({uspelo:false});
            ...
            if (err) response.send({uspelo:false})
                ...
                connection.query("INSERT INTO podjetja_eposta_blacklist (naslov) (SELECT eposta FROM podjetje WHERE idpodjetja = ?)",[request.body.idpodjetja],function(err,results,fields) {
                    console.log(err);
                    if (err) response.send({uspelo:false});
                    else {
    ...
    if (err) response.send({uspelo:false});
        ...
        if (err) response.send({uspelo:false})
            ...
            connection.query("UPDATE podjetje SET opozorjen = NULL,admin_pojasnilo = NULL,odgovor_podjetja = NULL WHERE idpodjetja = ?",[request.body.idpodjetja],function(err,results,fields) {
                console.log(err);
                if (err) response.send({uspelo:false});
