                        ...
                        if (err) response.send({uspelo:false})
                            ...
                            connection.query("SELECT IF (COUNT(*) > ?, true, false) AS pravilnost FROM spretnosti WHERE naziv IN (?);",[sanirane_spretnosti.length-1,sanirane_spretnosti],function(err,results,fields) {
                                ...
                                if (results[0].pravilnost == false) response.send({uspelo:'spretnost-narobe'});
                                    ...
                                    connection.query(
                                        `SELECT DISTINCT dela.idpodjetja, dela.opozorjen, dela.iddela, dela.naziv, dela.opis, placa, 
                                        nivojiizobrazbe.naziv AS nivoizobrazbe, podrocjapodjetji.imepodrocja, trajanje.naziv AS nazivtrajanja, 
                                        delovnik.naziv AS nazivdelovnika, vrste_plac.naziv AS nazivplace, GROUP_CONCAT(DISTINCT spretnosti.naziv SEPARATOR ', ') AS spretnosti 
                                        FROM dela 
                                        LEFT JOIN dela_opozorila ON dela.iddela = dela_opozorila.iddela 
                                        INNER JOIN podjetje ON podjetje.idpodjetja = dela.idpodjetja 
                                        LEFT JOIN dela_has_spretnosti ON dela.iddela = dela_has_spretnosti.iddela 
                                        INNER JOIN spretnosti ON dela_has_spretnosti.idspretnosti = spretnosti.idspretnosti 
                                        INNER JOIN izobrazba ON izobrazba.idnivoja = dela.idizobrazbe 
                                        INNER JOIN nivojiizobrazbe ON nivojiizobrazbe.idnivoja = dela.idizobrazbe AND izobrazba.idnivoja = nivojiizobrazbe.idnivoja
                                        INNER JOIN podrocjapodjetji ON podrocjapodjetji.idpodrocja = dela.idpodrocja 
                                        INNER JOIN trajanje ON trajanje.idtrajanja = dela.idtrajanja 
                                        INNER JOIN delovnik ON delovnik.iddelovnika = dela.iddelovnika 
                                        INNER JOIN vrste_plac ON vrste_plac.idplace = dela.idplace 
                                        LEFT JOIN dela_prijave ON dela.iddela = dela_prijave.iddela 
                                        LEFT JOIN podjetja_opozorila ON podjetja_opozorila.idpodjetja = dela.idpodjetja 
                                        WHERE (dela_opozorila.iddelavca != ? OR dela_opozorila.iddelavca IS NULL) 
                                        AND dela.naziv LIKE ? AND (podjetja_opozorila.iddelavca != ? 
                                        OR podjetja_opozorila.iddelavca IS NULL) 
                                        AND dela.idtrajanja LIKE ? AND dela.iddelovnika LIKE ? 
                                        AND dela.idplace LIKE ? AND podrocjapodjetji.imepodrocja LIKE ? 
                                        AND (dela_prijave.iddelavca != ? OR dela_prijave.iddelavca IS NULL) 
                                        AND dela.iddela IN (
                                            SELECT iddela 
                                            FROM dela_has_spretnosti 
                                            INNER JOIN spretnosti ON spretnosti.idspretnosti = dela_has_spretnosti.idspretnosti 
                                            WHERE naziv IN (?)) 
                                        AND (dela.opozorjen != 1 OR dela.opozorjen IS NULL) 
                                        AND (podjetje.opozorjen IS NULL) 
                                        AND (dela.placa BETWEEN IFNULL(?, 0) AND IFNULL(?, ~ 0)) 
                                ...
                                connection.query(
                                    `SELECT DISTINCT dela.idpodjetja,dela.opozorjen,dela.iddela, dela.naziv, dela.opis, placa, nivojiizobrazbe.naziv AS nivoizobrazbe, 
                                    podrocjapodjetji.imepodrocja, trajanje.naziv AS nazivtrajanja, delovnik.naziv AS nazivdelovnika,
                                    vrste_plac.naziv AS nazivplace, GROUP_CONCAT(DISTINCT spretnosti.naziv SEPARATOR ', ') AS spretnosti
                                    FROM dela 
                                    LEFT JOIN dela_opozorila ON dela.iddela = dela_opozorila.iddela 
                                    INNER JOIN podjetje ON podjetje.idpodjetja = dela.idpodjetja 
                                    LEFT JOIN dela_has_spretnosti ON dela.iddela = dela_has_spretnosti.iddela 
                                    ...
                                    LEFT JOIN dela_prijave ON dela.iddela = dela_prijave.iddela 
                                    LEFT JOIN podjetja_opozorila ON podjetja_opozorila.idpodjetja = dela.idpodjetja 
                                    WHERE (dela_opozorila.iddelavca != ? OR dela_opozorila.iddelavca IS NULL) AND dela.naziv LIKE ? 
                                    AND (podjetja_opozorila.iddelavca  != ? OR podjetja_opozorila.iddelavca IS NULL) 
                                    AND dela.idtrajanja LIKE ? AND dela.iddelovnika LIKE ? AND dela.idplace LIKE ? 
                                    AND podrocjapodjetji.imepodrocja LIKE ? AND (dela_prijave.iddelavca != ? 
                                    OR dela_prijave.iddelavca IS NULL) 
                                    AND (dela.opozorjen != 1 OR dela.opozorjen IS NULL) 
                                    AND (podjetje.opozorjen IS NULL) 
                                    AND (dela.placa BETWEEN ifnull(?,0) AND ifnull(?,~0)) 
                                    GROUP BY dela.iddela 
                                    ORDER BY dela.iddela ASC LIMIT 20;`,[request.session.userid,'%'+request.body.naziv+'%',request.session.userid,request.body.trajanje,request.body.delovnik,request.body.vrstaplace,request.body.podrocje,request.session.userid,request.body.minplaca,request.body.maxplaca],function(err,results,fields) {
                                    if (err) response.send({uspelo:false})
                                    else response.send({uspelo:true,dela:results});
                                });
                            }
...
router.get('/sprejet-dela',function(request,response) {
    ...
    connection.query("SELECT iddelavca FROM delavci WHERE iddelavca = ? AND opozorjen = 1",[request.session.userid],function(err,results,fields) {
        if (err) response.send({uspelo:false});
            ...
            `SELECT podjetje.idpodjetja,dela.iddela, dela.naziv, dela.opis, placa, nivojiizobrazbe.naziv AS nivoizobrazbe,
            podrocjapodjetji.imepodrocja, trajanje.naziv AS nazivtrajanja, delovnik.naziv AS nazivdelovnika, vrste_plac.naziv AS nazivplace,
            podjetje.eposta, podjetje.telefonska, GROUP_CONCAT(DISTINCT spretnosti.naziv SEPARATOR ', ') AS spretnosti
            FROM dela 
            LEFT JOIN dela_has_spretnosti ON dela.iddela = dela_has_spretnosti.iddela 
            LEFT JOIN spretnosti ON dela_has_spretnosti.idspretnosti = spretnosti.idspretnosti 
            INNER JOIN izobrazba ON izobrazba.idnivoja = dela.idizobrazbe 
            INNER JOIN nivojiizobrazbe ON nivojiizobrazbe.idnivoja = dela.idizobrazbe 
            AND izobrazba.idnivoja = nivojiizobrazbe.idnivoja 
            INNER JOIN podrocjapodjetji ON podrocjapodjetji.idpodrocja = dela.idpodrocja 
            INNER JOIN trajanje ON trajanje.idtrajanja = dela.idtrajanja 
            INNER JOIN delovnik ON delovnik.iddelovnika = dela.iddelovnika 
            INNER JOIN vrste_plac ON vrste_plac.idplace = dela.idplace 
            LEFT JOIN dela_prijave ON dela.iddela = dela_prijave.iddela 
            INNER JOIN podjetje ON podjetje.idpodjetja = dela.idpodjetja 
            WHERE dela_prijave.iddelavca = ? AND dela_prijave.sprejet = 1 
            GROUP BY dela.iddela 
            ORDER BY dela.iddela DESC;`,[request.session.userid],function(err,results,fields) {
        if (err) response.send({uspelo:false});
        else response.send({uspelo:true,dela:results})
    });
    });
